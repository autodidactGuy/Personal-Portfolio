...
import { parseContentIconId, type ContentIconId } from "@/config/content-icons";
...
function normalizeIconModule(
  ...
  const normalizedEntries = Object.entries(iconModule).filter(
    ([exportName, exportValue]) => exportName !== "default" && typeof exportValue === "function"
  );
...
async function loadIcon(iconId?: ContentIconId | null) {
  ...
  const loadPack = iconPackLoaders[parsedIcon.pack as keyof typeof iconPackLoaders];
