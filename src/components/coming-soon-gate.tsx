...
export function useComingSoonGate() {
	...
	const isCmsRoute = CMS_ROUTE_PREFIXES.some(
		(routePrefix) =>
			router.pathname.startsWith(routePrefix) ||
			router.asPath.startsWith(routePrefix),
	);
	const isComingSoonEnabled =
		siteConfig.comingSoonMode.enabled && !IS_LOCAL_DEVELOPMENT;
	const shouldShowComingSoon =
		isComingSoonEnabled && !cmsSession.isLoggedIn && !isCmsRoute;
